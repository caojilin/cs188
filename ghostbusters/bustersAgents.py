    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        #gonna get max prob for each ghosts
        maxGhosts = [max(beliefs, key=lambda x: beliefs[x])
                     for beliefs in livingGhostPositionDistributions]

        action = min(legal, key=lambda x: min([self.distancer.getDistance(
            Actions.getSuccessor(pacmanPosition, x), ghostPos)
                     for ghostPos in maxGhosts]))
        return action
